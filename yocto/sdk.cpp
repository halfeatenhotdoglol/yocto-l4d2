...
const handle_structs::class_groups::groups ssdk::c_base_entity::get_entity_group(ssdk::c_base_entity* ent)
{
	constexpr int boss[] = { handle_structs::class_ids::Tank, handle_structs::class_ids::Witch };
	constexpr int special[] = { handle_structs::class_ids::Boomer, handle_structs::class_ids::Charger, handle_structs::class_ids::Smoker, handle_structs::class_ids::Hunter, handle_structs::class_ids::Jockey, handle_structs::class_ids::Spitter };
	...
	else if (std::find(std::begin(special), std::end(special), class_id) != std::end(special))
		return handle_structs::class_groups::groups::SPECIAL;
	else if (std::find(std::begin(player), std::end(player), class_id) != std::end(player))
		return handle_structs::class_groups::groups::PLAYER;
	else if (std::find(std::begin(boss), std::end(boss), class_id) != std::end(boss))
